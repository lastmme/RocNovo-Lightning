...
def aa_match_batch(
    ...
    for peptide1, peptide2 in tqdm(zip(peptides1, peptides2), total=len(peptides1)):
...
def calculate_dataset_metrics(dir_path: Path, tokenizer: PTMPeptideTokenizer, beam_size: int):
    df = pd.DataFrame(columns=["species", "aa_prec", "aa_recall", "peptide_recall"])
    for f in dir_path.glob("*"):
        target_path = dir_path / f"{f.stem}" / f"{f.stem}_{beam_size}.npy"
        if not target_path.exists():
            continue
        print("-" * 50, f.stem, "-" * 50)
        result = np.load(target_path, allow_pickle=True)
        ...
        row.append(aa_precision)
        row.append(aa_recall)
        row.append(pep_recall)
        df.loc[len(df)] = row
